...
def magnet_coils(
    ...
    Arguments:
        ...
        width (float): width of coil cross-section (cm).
        thickness (float) thickness of coil cross-section (cm).
    ...
    coil_data = open(coils, 'r')
    data = coil_data.readlines()[start:stop]
    ...
    for line in data:
        ...
        if s == 0:
            ...
            cubit.cmd(
                'create surface rectangle width ' + str(thickness) + ' height '
                + str(width) + ' zplane'
            )
            ...
            # Determine angle by which to rotate cross-section in normal plane
            # such that its vector pointing in the direction of its thickness
            # faces the origin

            # Extract initial path point
            pos = cubit.vertex(path[0]).coordinates()

            # Cross-section inititally populated with thickness vector oriented
            # along x axis
            ...
            t_vec_perp_rot = t_vec_perp_mag*(rot_par*t_vec_perp + rot_perp*orth)
            ...
            # Get latest vertex, curve, and surface indices
