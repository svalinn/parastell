...
def offset_surface(vmec, theta, phi, offset):
    ...
    zeta = root_scalar(
        ...
        method = 'bisect', bracket = [phi - np.pi/4, phi + np.pi/4])
    zeta = zeta.root
...
def stellarator_torus(vmec, offset, cutter, num_periods, num_phi, num_theta):
    ...
    phi_list = np.linspace(0, np.pi/2, num = num_phi + 1)
    ...
    for phi in phi_list:
        ...
        for theta in theta_list:
            ...
            if offset == 0:
                ...
                x, y, z = vmec.vmec2xyz(1.0, theta, phi)
            
            else:
                # Compute offset surface point
                x, y, z = offset_surface(vmec, theta, phi, offset)
...
def parametric_stellarator(
    plas_eq, radial_build, num_periods, num_phi = 60, num_theta = 100,
    exclude = [], step_export = True, h5m_export = True, h5m_tags = None,
    include_graveyard = False, min_mesh_size = 5.0, max_mesh_size = 20.0,
    ...
    Arguments:
        ...
        radial_build (dict): lists component names and thicknesses in the
            ...
            in the order given.
        num_periods (int): number of stellarator periods to build in model.
        num_phi (int): number of phi geometric cross-sections to make (defaults
            to 60).
        num_theta (int): number of points defining the geometric cross-section
            (defaults to 100).
    ...
    plasma, cutter = stellarator_torus(vmec, offset, cutter, num_periods,
        num_phi, num_theta)
    ...
    for layer in radial_build:
        ...
        # Generate component
        torus, cutter = stellarator_torus(vmec, offset, cutter, num_periods,
            num_phi, num_theta)
